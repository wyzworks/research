# REF: https://github.com/lisette-espin/mrqap-python/blob/p3/libs/qap.py
...
class QAP():
    ...
    def correlation(self, x, y, show=True):
        ...
        #     # pc = np.corrcoef(x.flatten(), y.flatten())
        #     print(pc)
        #     # pc = np.corrcoef(x.flatten(), y.flatten())
        #     # print(pc)
        # else:
        ...
        #     # pc = np.corrcoef(x.flatten(), y.flatten())
        if show:
            utils.printf('Pearson Correlation: {}'.format(pc))
    ...
    def stats(self, x, y):
        xflatten = x.flatten()
        yflatten = y.flatten()
        p = np.corrcoef(xflatten,yflatten)[1]
